package com.chinamall21.mobile.kotlinstudy.utils

import android.util.Log
import com.chinamall21.mobile.kotlinstudy.api.RetrofitService
...
/**
 * desc：
 * author：Created by xusong on 2019/2/28 17:10.
 */

class RetrofitUtils {

    companion object {
        fun oncreate(url: String): Retrofit {
            //日志显示级别
            val level:HttpLoggingInterceptor.Level = HttpLoggingInterceptor.Level.BODY
            //新建log拦截器
            val loggingInterceptor: HttpLoggingInterceptor = HttpLoggingInterceptor(HttpLoggingInterceptor.Logger {
                message -> Log.e("RetrofitUtils", "OkHttp: " + message)
            })
        ...
        fun <T> getService(url: String, service: Class<T>): T {
            return oncreate(url).create(service)
        }
    }
}